...
function(a, b) {
	...
	b.addEventListener(a.EVENT_START, g), b.addEventListener(a.EVENT_MOVE, h), b.addEventListener(a.EVENT_END, i), b.addEventListener(a.EVENT_CANCEL, i), b.addEventListener(a.EVENT_CLICK, function(b) {
		...
		return this.each(function() {
			var e = this;
			e.addEventListener(b, function(f) {
				var g = a.qsa(c, e),
					h = f.target;
				if (g && g.length > 0)
					for (; h && h !== document && h !== e; h = h.parentNode)
						if (h && ~g.indexOf(h)) {
							if (~["click", "tap", "doubletap", "longtap", "hold"].indexOf(b) && (h.disabled || h.classList.contains("mui-disabled"))) break;
							f.detail ? f.detail.currentTarget = h : f.detail = {
								currentTarget: h
							}, d.call(h, f)
						}
			}), "tap" === b && (e.removeEventListener(a.EVENT_CLICK, j), e.addEventListener(a.EVENT_CLICK, j))
		})
	};
	var j = function(a) {
...
function(a, b) {
	a.os.plus && a.os.android && a.registerBack({
		...
		handle: function() {
			a.plusReady(function() {
				a.options.keyEventBind.backbutton || plus.key.removeEventListener("backbutton", a.back), a.options.keyEventBind.menubutton || plus.key.removeEventListener("menubutton", a.menu)
			})
		}
	})
}(mui, window),
	...
	var c = "mui-slider",
		...
		n = a.Scroll.extend({
			...
			_gotoItem: function(b, c) {
				this.currentPage = this._getPage(b, !0), this.scrollTo(this.currentPage.x, 0, c, this.options.bounceEasing), 0 === c && a.trigger(this.wrapper, "scrollend", this), this._initTimer()
			},
			setTranslate: function(a, b) {
				this._super(a, b);
				...
				if (b !== this.lastTranslateX) {
					if (this.slideIn) {
						...
						this.setTranslateX(b), b >= 0 ? (this.leftShowing = !0, this.rightShowing = !1, b > 0 && (this.offCanvasLeft && a.each(this.offCanvasLefts, function(a, b) {
							...
							console.log(b === this.offCanvasRight), b.style.zIndex = b === this.offCanvasRight ? 0 : -1
...
function(a, b, c) {
	...
	r.prototype.init = function() {
		...
		if (!f) {
			...
			f.className = b, b === this.searchActionClass && (f.innerHTML = '<span class="' + d + " " + g + '"></span>' + this.element.getAttribute("placeholder"), this.element.setAttribute("placeholder", ""), this.element.value.trim() && a.classList.add("mui-active")), a.insertBefore(f, this.element.nextSibling)
		}
		return f
...
function(a) {
	var b = "ontouchstart" in document,
		...
		i = a.Numbox = a.Class.extend({
			init: function(b, c) {
				var d = this;
				c = c || {}, c.step = parseInt(c.step || 1), d.options = c, d.holder = b, d.input = a.qsa("." + h, d.holder)[0], d.plus = a.qsa("." + f, d.holder)[0], d.minus = a.qsa("." + g, d.holder)[0], d.checkValue(), d.initEvent()
			},
			initEvent: function() {
				var b = this;
				b.plus.addEventListener(c, function() {
					var c = parseInt(b.input.value) + b.options.step;
					b.input.value = c.toString(), a.trigger(b.input, d, null)
				}), b.minus.addEventListener(c, function() {
					var c = parseInt(b.input.value) - b.options.step;
					b.input.value = c.toString(), a.trigger(b.input, d, null)
				}), b.input.addEventListener(d, function() {
					b.checkValue()
				})
			},
			checkValue: function() {
